...
shard_itr=$(aws --profile ${profile} kinesis get-shard-iterator --stream-name ${stream_name} --shard-id ${shard_id} --shard-iterator-type ${shard_itr_type} | jq ."ShardIterator" )
echo "found shard iterator for shard 0 ${shard_itr}"

limit=2
aws --profile ${profile} kinesis get-records --shard-iterator ${shard_itr} --limit ${limit} | jq .
